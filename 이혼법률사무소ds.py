...
import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns # 히트맵과 클러스터링을 위해 seaborn 사용
from google.colab import files
import numpy as np


#  Matplotlib 한글 폰트 설정 (Colab 환경)
# 한글 라벨링을 위해 폰트를 설정합니다. 깨지면 주석 라인을 실행 후 런타임 재시작

try:
    # Colab에 일반적으로 설치되어 있는 NanumGothic 폰트 사용 시도
    plt.rc('font', family='NanumGothic')
    plt.rcParams['axes.unicode_minus'] = False # 마이너스 기호 깨짐 방지
except:
    # 폰트가 없을 경우, 시각화는 되지만 한글이 깨질 수 있습니다.
    print("경고: NanumGothic 폰트를 찾을 수 없습니다. 한글 라벨이 깨질 수 있습니다.")
    print("해결하려면 코드 상단 주석 처리된 폰트 설치 라인을 실행 후 런타임을 재시작하세요.")
    plt.rcParams['axes.unicode_minus'] = False # 마이너스 기호 깨짐 방지
...
if not valid_features:
    print("QUESTION_MAP에 정의된 변수 중 데이터프레임에 존재하는 컬럼이 없습니다.")
    raise SystemExit("변수 불일치 오류")

corr_matrix = df[valid_features].corr()

# 긴 설문 문항 라벨로 인덱스/컬럼 이름 변경
...
# 클러스터 히트맵 생성 (클러스터링 적용)
# sns.clustermap을 사용하면 자동으로 계층적 클러스터링을 적용하여 유사한 변수끼리 묶을 수 있음
sns.clustermap(
    corr_matrix,
    annot=False,
    cmap='coolwarm',
...
plt.suptitle("설문 문항 간 유사도 (상관관계) 클러스터 히트맵", y=1.02, fontsize=18, fontweight='bold')
